...
// --- UPDATED LOGIC: Fairer Grading System based on uploaded files ---
const QUICK_ISSUES = {
  BED: [
    // GRADE A: 嚴重客訴/不通過 (Severe/Fail)
    { label: '遺留物/垃圾', grade: 'A', desc: '含衣物/垃圾/自身用品 (考核:不通過)', color: 'border-red-900/20 bg-red-50 text-red-900' },
    { label: '毛髮/碎屑/蟲屍', grade: 'A', desc: '地板/抽屜明顯可見 (考核:不通過)', color: 'border-red-900/20 bg-red-50 text-red-900' },
    { label: '備品全空/未補', grade: 'A', desc: '器皿/拖鞋/水完全沒補 (考核:不通過)', color: 'border-red-900/20 bg-red-50 text-red-900' },
    { label: '冰箱/器皿髒污', grade: 'A', desc: '內部污漬/杯具有水痕 (考核:不通過)', color: 'border-red-900/20 bg-red-50 text-red-900' },
    { label: '陽台/戶外區髒污', grade: 'A', desc: '家具/地板/欄杆積塵 (考核:不通過)', color: 'border-red-900/20 bg-red-50 text-red-900' },
    { label: '鋪床髒污/破損', grade: 'A', desc: '床單/被套有明顯污漬 (考核:不通過)', color: 'border-red-900/20 bg-red-50 text-red-900' },
...
    { label: '衣架/保險箱失誤', grade: 'C', desc: '數量錯誤/收納不整 (考核:-2分)', color: 'border-yellow-900/20 bg-yellow-50 text-yellow-900' },
...
    { label: '衛生紙/備品微調', grade: 'C', desc: '無三角形/未補滿/微污 (考核:-2分)', color: 'border-yellow-900/20 bg-yellow-50 text-yellow-900' },
  ],
  WATER: [
    // GRADE A: 嚴重客訴/不通過 (Severe/Fail)
...
    { label: '玻璃杯嚴重水痕', grade: 'A', desc: '光照下明顯水痕 (考核:不通過)', color: 'border-red-900/20 bg-red-50 text-red-900' },
...
  const [historyEditTarget, setHistoryEditTarget] = useState(null); // { docId, type: 'bed'|'water' }
...
    setCurrentIssue({ team: activeTab, title: tpl.label, grade: tpl.grade, note: '', photo: null });
...
    setStaffModalTarget(type); // Re-use staffModalTarget logic for list selection
    setStaffSearch('');
    setIsEditMode(false);
...
    
    // Soft validation for bed/water staff
    let confirmMsg = "";
    if (issues.length === 0) {
...
      bedStaff: bedStaff || '', // Allow empty
      waterStaff: waterStaff || '', // Allow empty
      issues, 
...
                  <div className="flex justify-between items-start mb-2"><span className="font-bold text-sm leading-tight text-[#2C2C2C]">{issue.label}</span></div>
                  <div className="flex justify-between items-end"><span className="text-[10px] text-[#666] leading-tight w-2/3">{issue.desc}</span><span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border bg-white/80 backdrop-blur-sm shadow-sm ${issue.grade === 'A' ? 'text-red-800 border-red-200' : issue.grade === 'B' ? 'text-orange-700 border-orange-200' : 'text-yellow-700 border-yellow-200'}`}>{issue.grade}</span></div>
                </button>
              ))}
              <button onClick={() => setCurrentIssue({ team: activeTab, title: '', grade: 'C', note: '', photo: null }) || setShowIssueModal(true)} className="p-4 rounded-xl border-2 border-dashed border-[#DDD] flex flex-col items-center justify-center text-[#999] hover:bg-white hover:border-[#BBB] transition-colors"><Plus size={24} className="mb-1 opacity-50" /><span className="text-xs font-bold tracking-widest">自定義缺失</span></button>
            </div>
          </div>
          {issues.length > 0 && (
...
                {issues.map(i => (
                  <div key={i.id} className="bg-white p-3 rounded-xl border border-[#F0F0F0] shadow-sm flex items-start gap-3">
...
                    <div className="flex-1 min-w-0 py-1"><div className="flex justify-between items-start"><h4 className="font-bold text-[#2C2C2C] text-sm truncate">{i.title}</h4><span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ml-2 flex-shrink-0 ${(ERROR_GRADES[i.grade] || ERROR_GRADES['C']).color}`}>{i.grade}級</span></div><p className="text-xs text-[#888] mt-1 line-clamp-2">{i.note || "無補充說明"}</p></div>
...
              {!currentIssue.title && <input className="w-full p-4 bg-white border border-[#E0E0E0] rounded-xl font-bold text-[#2C2C2C] focus:border-[#8B5E3C] outline-none" placeholder="輸入缺失名稱..." value={currentIssue.title} onChange={e => setCurrentIssue({...currentIssue, title: e.target.value})} />}
              <div>
...
        {roomsData.map(r => (
          <div key={r.id} className="bg-white p-5 rounded-2xl border border-[#F0F0F0] shadow-sm hover:shadow-md transition-shadow">
...
              ) : (
                <button onClick={() => openHistoryStaffEdit(r.id, 'bed')} className="text-xs text-indigo-500 font-bold border border-indigo-200 px-2 py-0.5 rounded bg-white hover:bg-indigo-50 flex items-center gap-1"><HelpCircle size={10}/> 補填床組</button>
              )}
              <div className="h-4 w-[1px] bg-[#DDD]"></div>
              {r.waterStaff ? (
                <span className="text-xs">水: {String(r.waterStaff)}</span>
              ) : (
                <button onClick={() => openHistoryStaffEdit(r.id, 'water')} className="text-xs text-indigo-500 font-bold border border-indigo-200 px-2 py-0.5 rounded bg-white hover:bg-indigo-50 flex items-center gap-1"><HelpCircle size={10}/> 補填水組</button>
              )}
            </div>
