...
const appId = 'hoshinoya-inspection-v3';
            ...
            <div className="flex-1 overflow-y-auto pb-32 p-5 space-y-6">
              ...
              {issues.length > 0 && (
                <div className="space-y-3">
                  ...
                  {issues.map(i => (
                    <div key={i.id} className={`bg-white p-3 rounded-xl border-l-4 shadow-sm flex items-start gap-3 ${TEAMS_INFO[(i.team || 'water').toUpperCase()]?.borderColor}`}><div className="flex-1"><span className={`text-[9px] font-bold uppercase ${i.team === 'water' ? 'text-[#4A6C6F]' : 'text-[#8B5E3C]'}`}>{i.team === 'water' ? '水組' : '床組'}</span><h4 className="font-bold text-[#2C2C2C] text-sm">{i.title}</h4><p className="text-xs text-[#888]">{i.note || "無備註"}</p></div><button onClick={() => setIssues(issues.filter(x => x.id !== i.id))} className="text-[#DDD] hover:text-red-500"><X size={20} /></button></div>
                  ))}
                </div>
              )}
            </div>
        ...
        {view === 'analytics' && (
          <div className="flex flex-col h-full bg-[#F5F5F0]">
            ...
            <div className="p-6 space-y-6 overflow-y-auto pb-20">
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-5 rounded-2xl shadow-sm border"><p className="text-xs text-[#888] font-bold uppercase tracking-widest flex items-center gap-1"><MapPin size={12}/> 本月檢查</p><p className="text-4xl font-serif mt-2 font-bold">{stats.count}</p></div>
                    <div className="bg-white p-5 rounded-2xl shadow-sm border"><p className="text-xs text-[#888] font-bold uppercase tracking-widest flex items-center gap-1"><AlertTriangle size={12}/> 缺失總計</p><p className="text-4xl font-serif text-[#8B3A3A] mt-2 font-bold">{stats.total}</p></div>
                </div>
                <div className="bg-white p-6 rounded-2xl border shadow-sm">
                    <h3 className="font-serif font-bold text-sm mb-5 flex items-center gap-2"><BarChart2 size={18} className="text-[#8B5E3C]"/> 本月缺失排行</h3>
      ...
      {showRoomModal && (
        <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-md flex items-end sm:items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl h-[80vh] flex flex-col overflow-hidden font-sans">
            <div className="flex justify-between items-center mb-6"><div><h3 className="text-xl font-serif font-bold tracking-widest">請選擇房號</h3><p className="text-xs text-gray-400 font-sans uppercase">Select a room</p></div><button onClick={() => setShowRoomModal(false)} className="p-2 bg-gray-100 rounded-full"><X size={20}/></button></div>
            <div className="flex-1 overflow-y-auto space-y-8 pr-2 pb-10">
                {FLOORS_DATA.map(f => (
                    <div key={f.floor}>
                        <div className="flex items-center gap-3 mb-4 sticky top-0 bg-white py-1 z-10"><span className="bg-[#2C2C2C] text-white text-xs font-bold px-3 py-1 rounded-full">{f.floor}F</span><div className="h-[1px] flex-1 bg-gray-100"></div></div>
          ...
          <div className="bg-[#F9F9F9] w-full max-w-md rounded-t-3xl p-6 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10">
            ...
            <div className="flex gap-2">
              <label className="flex-1 bg-white border-2 border-dashed border-[#DDD] rounded-xl py-4 flex flex-col items-center justify-center cursor-pointer h-24 overflow-hidden relative active:bg-gray-50 transition-colors">
                {currentIssue.photo ? <img src={currentIssue.photo} className="h-full w-full object-cover" onClick={async (e) => { e.preventDefault(); const rect = e.target.getBoundingClientRect(); const x = (e.clientX - rect.left) * (720/rect.width); const y = (e.clientY - rect.top) * (720/rect.width); const newPhoto = await drawCircleOnImage(currentIssue.photo, x, y); setCurrentIssue({...currentIssue, photo: newPhoto}); }} /> : <div className="flex flex-col items-center"><Camera className="text-[#AAA]" size={20}/><span className="text-[9px] font-bold text-[#AAA] mt-1">拍照/上傳</span></div>}
        ...
        <div className="fixed inset-0 z-[150] bg-black/60 flex items-end sm:items-center justify-center backdrop-blur-sm font-sans">
          <div className="bg-[#F9F9F9] w-full max-w-md h-[80vh] rounded-t-3xl flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-gray-50">
               ...
               {(!selectedHistoryItem.issues || selectedHistoryItem.issues.length === 0) && <div className="text-center py-20 text-[#CCC] font-bold tracking-widest flex flex-col items-center gap-3"><CheckCircle size={48} className="opacity-20"/><p>無缺失紀錄 (PASS)</p></div>}
            </div>
          </div>
        </div>
      )}
